{{-- setting --}}

@props(
[
    'content'   => null,
    'iconClass' => null,
    'textClass' => null,
])

{{-- classes --}}

@php $classes = ObsidianUI::classes()
        ->add('[:where(&)]:w-full')
        ->add('[:where(&)]:relative')
        ->add('[:where(&)]:max-h-[inherit]')
        ->add('[:where(&)]:overflow-hidden');
@endphp

{{-- classes --}}

@php $textclasses = ObsidianUI::classes()
        ->add('[:where(&)]:px-8')
        ->add('[:where(&)]:w-full')
        ->add('[:where(&)]:h-full')
        ->add('[:where(&)]:text-xl')
        ->add('[:where(&)]:italic')
        ->add('[:where(&)]:max-h-[inherit]')
        ->add('[:where(&)]:overflow-auto')
        ->add($textClass);
@endphp

{{-- output --}}

<div {{ $attributes->class($classes) }} data-obsidian-ui-quote>

    <div class="absolute top-0 left-0 opacity-80 {{ $iconClass }}" data-obsidian-ui-quote-icon>
        <svg class="scale-x-[-1]" fill="currentColor" height="20px" width="20px" viewBox="0 0 512 512">
        	<path d="M119.472,66.59C53.489,66.59,0,120.094,0,186.1c0,65.983,53.489,119.487,119.472,119.487   c0,0-0.578,44.392-36.642,108.284c-4.006,12.802,3.135,26.435,15.945,30.418c9.089,2.859,18.653,0.08,24.829-6.389   c82.925-90.7,115.385-197.448,115.385-251.8C238.989,120.094,185.501,66.59,119.472,66.59z"/>
	        <path d="M392.482,66.59c-65.983,0-119.472,53.505-119.472,119.51c0,65.983,53.489,119.487,119.472,119.487   c0,0-0.578,44.392-36.642,108.284c-4.006,12.802,3.136,26.435,15.945,30.418c9.089,2.859,18.653,0.08,24.828-6.389   C479.539,347.2,512,240.452,512,186.1C512,120.094,458.511,66.59,392.482,66.59z"/>
        </svg>
    </div>

    <div class="absolute bottom-0 right-0 opacity-80 {{ $iconClass }}" data-obsidian-ui-quote-icon>
        <svg fill="currentColor" height="20px" width="20px" viewBox="0 0 512 512">
	        <path d="M119.472,66.59C53.489,66.59,0,120.094,0,186.1c0,65.983,53.489,119.487,119.472,119.487   c0,0-0.578,44.392-36.642,108.284c-4.006,12.802,3.135,26.435,15.945,30.418c9.089,2.859,18.653,0.08,24.829-6.389   c82.925-90.7,115.385-197.448,115.385-251.8C238.989,120.094,185.501,66.59,119.472,66.59z"/>
	        <path d="M392.482,66.59c-65.983,0-119.472,53.505-119.472,119.51c0,65.983,53.489,119.487,119.472,119.487   c0,0-0.578,44.392-36.642,108.284c-4.006,12.802,3.136,26.435,15.945,30.418c9.089,2.859,18.653,0.08,24.828-6.389   C479.539,347.2,512,240.452,512,186.1C512,120.094,458.511,66.59,392.482,66.59z"/>
        </svg>
    </div>

    <div class="{{ $textclasses }}" data-obsidian-ui-quote-text>{{ $slot }}</div>

</div>